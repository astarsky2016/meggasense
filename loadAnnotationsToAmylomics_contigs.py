...
def __mapIdtoKegg(data, dna):
    ...
    x = HmmerTabParser(data, dna, e_threshold = 10, overlap_limit = 0.5, infoLine = re.compile("\S"), startLine = Keyword("# <seq id>"))
    for met_id, item in x.hmmerAnnotation.items():
        [koMap[el[2]].append((met_id.split("_")[0], "From-To:", el[0], "-", el[1]) + ("E-value:", el[3]) + ("Score:", el[4]) + ("Orientation:", el[5])) for el in item]
    return(dict([(key, sorted(item, key=lambda x: x[6])) for key, item in koMap.items()])) 
def load(core, data, schema, hmm, dna):
    koMap = __mapIdtoKegg(data, dna)
    undetermined = 0
    solr_interface = sunburnt.SolrInterface(core, schema)
    ids = ['kegg_orthologue', 'EC_number', 'CAZy_modules','gene_hits', 'name','definition','class','pathway','pmid', 'global', 'specific', 'cazy_activities', 'cazy_specificities']   
    for ko, item in koMap.items():
        document = {}
        document["kegg_orthologue"] = ko
        document["gene_hits"] = tuple(" ".join([str(el) for el in metId]) for metId in item)
...
def main():
    ...
    for hmm in hmmer_files:
        target = hmm.split(".")[0] + "_contigs"
        core = coreURL + target
        schema = os.path.join(coreDir, coreMap[target], "conf/schema.xml")
        print schema
        print core
        data = os.path.join(dataDir, hmm)
        contigDna = os.path.join(dataDir, hmm.split(".")[0] + ".fna")
        load(core, data, schema, hmm, contigDna)
